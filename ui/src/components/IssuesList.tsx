...
export function IssuesList({
  ...
  const { t } = useTranslation(["issuesList", "status"]);
  const quickFilterPresets = useMemo(
    () => [
      { label: t("issuesList:quickAll"), statuses: [] as string[] },
      { label: t("issuesList:quickActive"), statuses: ["todo", "in_progress", "in_review", "blocked"] },
      { label: t("issuesList:quickBacklog"), statuses: ["backlog"] },
      { label: t("issuesList:quickDone"), statuses: ["done", "cancelled"] },
    ],
    [t],
  );
  const labelForIssueStatus = useCallback(
    (s: string) => t(`issuesList:statusLabel.${s}`, { defaultValue: s.replace(/_/g, " ") }),
    [t],
  );
  const labelForPriority = useCallback(
    (p: string) => t(`status:priority.${p}`, { defaultValue: p }),
    [t],
  );
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return statusOrder
        ...
        .map((s) => ({ key: s, label: labelForIssueStatus(s), items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return priorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: labelForPriority(p), items: groups[p]! }));
    }
    // assignee
  ...
  }, [filtered, viewState.groupBy, agents, agentName, currentUserId, labelForIssueStatus, labelForPriority, t]);
  ...
  return (
    <div className="space-y-4">
            ...
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">
                  {activeFilterCount > 0
                    ? t("issuesList:filterCount", { count: activeFilterCount })
                    : t("issuesList:filter")}
                </span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t("issuesList:filtersHeading")}</span>
                  {activeFilterCount > 0 && (
                    <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-muted-foreground">{t("issuesList:statusSection")}</span>
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{labelForIssueStatus(s)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("issuesList:prioritySection")}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{labelForPriority(p)}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Assignee */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("issuesList:assigneeSection")}</span>
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issuesList:labelsSection")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issuesList:projectSection")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {(["status", "priority", "title", "created", "updated"] as const).map((field) => (
                    ...
                    >
                      <span>{t(`issuesList:sortField.${field}`)}</span>
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {(["status", "priority", "assignee", "none"] as const).map((value) => (
                    <button
                      key={value}
                      className={`flex items-center justify-between w-full px-2 py-1.5 text-sm rounded-sm ${
                    ...
                    >
                      <span>{t(`issuesList:groupBy.${value}`)}</span>
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issuesList:emptyFiltered")}
