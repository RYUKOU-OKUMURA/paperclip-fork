...
function formatLastRunTimestamp(value: Date | string | null | undefined) {
  if (!value) return i18n.t("routines:never");
...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel =
          left === "__no_project"
            ? i18n.t("routines:noProject")
            : (projectById.get(left)?.name ?? i18n.t("routines:unknownProject"));
        const rightLabel =
          right === "__no_project"
            ? i18n.t("routines:noProject")
            : (projectById.get(right)?.name ?? i18n.t("routines:unknownProject"));
      ...
      .map((key) => ({
        ...
        label:
          key === "__no_project"
            ? i18n.t("routines:noProject")
            : (projectById.get(key)?.name ?? i18n.t("routines:unknownProject")),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel =
        left === "__unassigned"
          ? i18n.t("routines:unassigned")
          : (agentById.get(left)?.name ?? i18n.t("routines:unknownAgent"));
      const rightLabel =
        right === "__unassigned"
          ? i18n.t("routines:unassigned")
          : (agentById.get(right)?.name ?? i18n.t("routines:unknownAgent"));
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label:
        key === "__unassigned"
          ? i18n.t("routines:unassigned")
          : (agentById.get(key)?.name ?? i18n.t("routines:unknownAgent")),
      items: groups[key]!,
    }));
}
...
function RoutineListRow({
  ...
  const { t } = useTranslation(["routines", "actions"]);
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused") ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines:statusArchivedShort") : t("routines:statusPausedShort")}
            </span>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-2">
            ...
            />
            <span>{project?.name ?? t("routines:unknownProject")}</span>
          </span>
          <span className="flex items-center gap-2">
            {agent?.icon ? <AgentIcon icon={agent.icon} className="h-3.5 w-3.5 shrink-0" /> : null}
            <span>{agent?.name ?? t("routines:unknownAgent")}</span>
          </span>
          <span>
      ...
      <div className="flex items-center gap-3" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={
              enabled
                ? t("routines:ariaDisableRoutine", { title: routine.title })
                : t("routines:ariaEnableRoutine", { title: routine.title })
            }
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines:rowArchived") : enabled ? t("routines:toggleOn") : t("routines:toggleOff")}
          </span>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            >
              {runningRoutineId === routine.id ? t("routines:runningShort") : t("routines:runNow")}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
...
export function Routines() {
  const { t } = useTranslation(["routines", "actions", "common"]);
  ...
  const concurrencyPolicyDescriptions: Record<string, string> = useMemo(
    () => ({
      coalesce_if_active: t("routines:concurrencyPolicyDescriptions.coalesce_if_active"),
      always_enqueue: t("routines:concurrencyPolicyDescriptions.always_enqueue"),
      skip_if_active: t("routines:concurrencyPolicyDescriptions.skip_if_active"),
    }),
    [t],
  );
  const catchUpPolicyDescriptions: Record<string, string> = useMemo(
    () => ({
      skip_missed: t("routines:catchUpPolicyDescriptions.skip_missed"),
      enqueue_missed_with_cap: t("routines:catchUpPolicyDescriptions.enqueue_missed_with_cap"),
    }),
    [t],
  );
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines:breadcrumb") }]);
  }, [setBreadcrumbs, t]);
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("routines:groupBy")}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ...
                    ["assignee", t("routines:groupAssignee")],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines:composerTitle")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines:composerSubtitle")}
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routines:inlineAssignee")}</span>
                  ...
                  />
                  <span>{t("routines:inlineProject")}</span>
                  <InlineEntitySelector
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines:advancedDeliveryTitle")}</p>
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {formatErrorForUser(createRoutine.error)}
                </p>
              ) : null}
            </div>
          </div>
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines:emptyMessage")}
              />
            </div>
          ) : (
